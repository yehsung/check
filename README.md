...
팀 카드 헤더의 트로피 버튼을 누르면 **이번 주 팀 리그** 페이지가 열립니다. 팀별 이번 주(월요일 KST 기준) 총 근무시간을 내림차순으로 겨루는 순위표로, 각 팀의 총시간·주간 목표 대비 진행률·근무중 인원과 함께 우리 팀 행이 하이라이트됩니다. 순위는 `team_weekly_leaderboard()` RPC(로그인 팀원 전용)로 받아 오며 invite_code 는 노출하지 않습니다.

## 범위

- 포함: 상단바 근무중 표시, 근무 타이머, 팀원 상태, 오늘 근무시간, 팀 리그(주간 총근무시간 순위), Supabase Auth/REST 동기화, RLS 기반 스키마
