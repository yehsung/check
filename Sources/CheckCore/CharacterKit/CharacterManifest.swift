...
package struct CharacterManifest: Codable, Equatable, Sendable {
    ...
    package struct Atlas: Codable, Equatable, Sendable {
        ...
        package let states: [String: State]
    }

    package struct State: Codable, Equatable, Sendable {
        /// **재생 순서 그대로**. 같은 rect 가 여러 번 와도 된다(여우 걷기 = 0,1,2,1 — 픽스처 실측 결정).
        package let frames: [Rect]
        /// 프레임별 노출 시간(밀리초). `frames` 와 길이가 같아야 한다.
        package let durationsMs: [Int]
        /// 끝에서 처음으로 돌아가는가. false 면 마지막 프레임에서 멈춘다.
        package let loop: Bool

        package init(frames: [Rect], durationsMs: [Int], loop: Bool) {
            self.frames = frames
    ...
    package struct Rect: Codable, Equatable, Sendable {
        package let x: Int
        package let y: Int
        package let w: Int
        package let h: Int
    ...
    package struct Portrait: Codable, Equatable, Sendable {
        package let neutral: String
        package let negative: String

        package init(neutral: String, negative: String) {
    ...
    package enum StateKey {
        /// 정면 idle. **평면 크기의 기준**이라 스프라이트에 반드시 있어야 한다.
        package static let frontIdle = "frontIdle"
        /// 90° 옆모습 idle(방향만 잡고 멈춰 있을 때).
        package static let sideIdle = "sideIdle"
        /// 90° 옆모습 걷기(드래그로 이동 중).
        package static let sideWalk = "sideWalk"
    }
    ...
    package init(id: String, displayName: String, kind: Kind, atlas: Atlas? = nil, portrait: Portrait? = nil,
         pixelArt: Bool? = nil) {
        self.id = id
...
package enum CharacterManifestError: Error, Equatable, CustomStringConvertible {
