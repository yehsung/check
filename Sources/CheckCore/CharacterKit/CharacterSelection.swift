...
@MainActor
package final class CharacterSelection {
    /// UserDefaults 키. 마이그레이션·진단이 이 이름을 그대로 쓴다.
    package static let defaultsKey = "check.character.selected"

    private let defaults: UserDefaults
    private let catalog: CharacterCatalog

    package init(defaults: UserDefaults, catalog: CharacterCatalog) {
        self.defaults = defaults
        self.catalog = catalog
    }

    /// 현재 선택. 저장값이 없거나 카탈로그에 없으면 아잉.
    package var selectedID: String {
    ...
    package var selectedManifest: CharacterManifest {
    ...
    @discardableResult
    package func select(_ id: String) -> Bool {
