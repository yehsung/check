...
@MainActor
package enum SpriteCharacterNode {
    ...
    package nonisolated static let targetExtent: CGFloat = 1.9210

    /// 노드 이름. 나중에 씬에서 스프라이트 평면을 다시 찾기 위한 열쇠(프레임 교체·미러가 이 노드에 걸린다).
    package static let nodeName = "check.spriteCharacter"
    ...
    package static func make(manifest: CharacterManifest, atlas: CGImage) -> SCNNode? {
    ...
    package static func apply(frame: CharacterManifest.Rect, mirrored: Bool, to node: SCNNode, atlasSize: CGSize) {
    ...
    package nonisolated static func planeSize(for frame: CharacterManifest.Rect) -> CGSize {
    ...
    package nonisolated static func contentsTransform(
        ...
        transform.m11 = SCNFloat(mirrored ? -scaleX : scaleX)
        transform.m22 = SCNFloat(scaleY)
        transform.m41 = SCNFloat(offsetX)
        transform.m42 = SCNFloat(offsetY)
        return transform
    }
    ...
    package nonisolated static func atlasUV(
