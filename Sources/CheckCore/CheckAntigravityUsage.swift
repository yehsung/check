...
package enum AntigravityProto {
    /// base-128 varint. 버퍼 끝이거나 10바이트를 넘으면 nil(= 깨진 메시지) — 호출자는 그 메시지를 통째로 버린다.
    package static func readVarint(_ b: UnsafeRawBufferPointer, _ i: inout Int, _ end: Int) -> UInt64? {
        var result: UInt64 = 0
        var shift: UInt64 = 0
        while i < end {
            let byte = b[i]
...
package struct AntigravityGenRow: Equatable, Sendable {
    package var input: Int = 0
    package var output: Int = 0
    package var thinking: Int = 0
    ...
    package var total: Int { input + output + thinking + cacheRead }
}
...
package enum AntigravityGenMetadataParser {
    // 최상위 → 생성 결과 메시지.
    package static let genField = 1
    // 생성 메시지 안: 사용량 정본 / 미러(17 → 2) / 모델 식별자.
    package static let usageField = 4
    package static let usageMirrorOuterField = 17
    package static let usageMirrorInnerField = 2
    package static let modelField = 19
    // 사용량 메시지 안의 토큰 네 갈래.
    package static let inputTokensField = 2
    package static let outputTokensField = 3
    package static let cacheReadTokensField = 5
    package static let thinkingTokensField = 9
    ...
    package static let maxPlausibleTokens = 100_000_000

    /// 모델 식별자로 받아줄 최대 길이. 이보다 길면 모델 필드가 아니라 본문이 흘러든 것이다 → 버린다.
    package static let maxModelIdentifierBytes = 64

    /// 실패(파싱 깨짐 / 정상성 검사 탈락 / 사용량 메시지 없음)면 nil. 호출자는 그 행을 건너뛴다.
    package static func parse(_ b: UnsafeRawBufferPointer) -> AntigravityGenRow? {
        var gen: Range<Int>?
    ...
    package static func parse(_ bytes: [UInt8]) -> AntigravityGenRow? {
...
package enum AntigravityConversationReader {
    ...
    package static let maxRowsPerScan = 1024
...
package struct AntigravityFileProgress: Equatable, Sendable {
    ...
    package var monthThinking: Int
    package var monthCacheRead: Int
    ...
    package var modelContrib: [String: Int]
    ...
    package var monthTotal: Int { monthInput + monthOutput + monthThinking + monthCacheRead }
}
...
package struct AntigravityUsageTotals: Equatable, Sendable {
    package var input: Int = 0
    package var output: Int = 0
    package var thinking: Int = 0
    ...
    package var daily: [String: Int] = [:]
    /// 모델 식별자 → total. 도구 믹스용 보조 지표.
    package var models: [String: Int] = [:]
...
package enum AntigravityUsageScanner {
    /// 홈 아래 대화 디렉터리의 상대 경로.
    package static let conversationsSubpath = ".gemini/antigravity-cli/conversations"

    /// 파일 상태 보관 기간. 월이 아니라 90일인 이유는 AntigravityFileProgress 주석 참고 —
    /// lastIdx 를 잃으면 그 대화를 통째로 재계상한다.
    package static let stateRetention: TimeInterval = 90 * 24 * 3_600

    package struct Stats: Equatable, Sendable {
        package var filesStatted = 0
        package var filesRead = 0
        package var rowsIngested = 0
        package var rowsRejected = 0
        /// **열기** 실패(삭제·권한·경로가 디렉터리 — 2단까지 `open_v2` 가 실패, 그리고 읽는 중 파일이 바뀐 경우).
        /// 재시도로 풀릴 수 있다.
        package var openFailures = 0
    ...
    package static func totals(_ states: [String: AntigravityFileProgress], month: String) -> AntigravityUsageTotals {
    ...
    @discardableResult
    package static func evict(_ states: inout [String: AntigravityFileProgress], now: Date) -> Bool {
