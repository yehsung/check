...
/// 있어 내용은 읽지 않는다).
package struct CodexAccountUsage: Codable, Equatable, Sendable {
    /// 이 스냅샷을 받은 시각(앱 시계).
    package var fetchedAt: Date
    /// 계정 누적 토큰(`summary.lifetimeTokens`). 프로토콜상 null 일 수 있다.
    package var lifetimeTokens: Int?
    /// UTC 'YYYY-MM-DD' → 그 날 토큰. 최근 `retentionDays` 일만 보관한다(스냅샷이 UserDefaults 에 영속되므로 무한히 자라지 않게).
    package var buckets: [String: Int]

    /// 버킷 보관 일수. 월 롤오버 직후에도 지난달 전체가 남아 있게 두 달을 넉넉히 덮는 70일.
    package static let retentionDays = 70

    package init(fetchedAt: Date, lifetimeTokens: Int?, buckets: [String: Int]) {
    ...
    package func monthTotal(_ month: String) -> Int {
        let prefix = month + "-"
        var sum = 0
    ...
    package var latestBucketDate: String? { buckets.keys.max() }

    /// 그 월 안에서 가장 최근 버킷 날짜. 툴팁의 "D일까지 반영" 이 쓴다 — 지난달 버킷을 이번 달 반영일로 오인하지 않게 월을 가른다.
    package func latestBucketDate(in month: String) -> String? {
        let prefix = month + "-"
    ...
    package func pruned() -> CodexAccountUsage {
    ...
    package static func utcDayString(_ date: Date) -> String {
...
package enum CodexAccountProbeStatus: Int, Codable, Equatable, Sendable {
    /// 계정 사용량을 받았다.
    case ok = 1
...
#if os(macOS)
// B3: codex app-server 를 자식 프로세스(Process)로 띄워 계정 사용량을 묻는다 — macOS 전용.
...
package enum CodexAccountUsageProbe {
    /// 프로브 실패. status 가 서버에 올라가고 reason 은 로컬 진단용(서버에 보내지 않는다 — 문자열은 어디에도 싣지 않는다).
    package struct Failure: Error, Equatable, Sendable {
        package var status: CodexAccountProbeStatus
    ...
    package final class LocateCache: Sendable {
        fileprivate let state = OSAllocatedUnfairLock(initialState: LocateState())
        package static let shared = LocateCache()
        package init() {}

        /// 실행으로 확정된 툴체인(없으면 nil). 테스트 관측용.
        package var confirmedToolchain: Toolchain? { state.withLock { $0.toolchain } }
        /// 후보 전부 실패/부재가 캐시된 상태(status)와 시각. 테스트 관측용.
        package var failure: (status: CodexAccountProbeStatus, at: Date)? {
    ...
    package static func cachedCodexHome(cache: LocateCache = .shared) -> URL? {
    ...
    package static func resolveCodexHome(
    ...
    package static func resolveShellEnvironment(
        now: Date = Date(), cache: LocateCache = .shared, lookup: ShellLookup? = nil
    ...
    package static func needsInteractiveLookup(_ login: ShellEnvironment?) -> Bool {
    ...
    package static func merged(login: ShellEnvironment?, interactive: ShellEnvironment) -> ShellEnvironment {
    ...
    package static func fallbackCandidates(homeDirectory: URL) -> [URL] {
    ...
    package static func nvmBinDirectories(homeDirectory: URL) -> [String] {
    ...
    package static func nodeSearchDirectories(homeDirectory: URL) -> [String] {
        let home = homeDirectory
    ...
    package static func candidateToolchains(
    ...
    package static var shellLookupScript: String {
    ...
    package static func parseShellLookup(_ data: Data) -> ShellEnvironment? {
    ...
    package static func requestLines(appVersion: String) -> [String] {
    ...
    package static func parse(lines: [String], requestID: Int = usageRequestID, fetchedAt: Date) -> Result<CodexAccountUsage, Failure> {
        for raw in lines {
    ...
    package static func confirmsToolchain(_ result: Result<CodexAccountUsage, Failure>) -> Bool {
        switch result {
    ...
    package enum SessionKind: Sendable { case responded, exited, timeout, launchFailed }
    ...
    package static func drainNonBlocking(_ fd: Int32) -> Data {
    ...
    package static func lineHasUsageResponseID(_ line: String) -> Bool {
...
#if os(macOS)
// B3: 위 프로브의 러너 타입(CodexAccountUsageProbe.Failure)에 묶인 맥 스토어 — macOS 전용.
...
@Observable
@MainActor
package final class CodexAccountUsageStore {
    package typealias Runner = @Sendable (_ homeDirectory: URL, _ now: Date) async -> Result<CodexAccountUsage, CodexAccountUsageProbe.Failure>
    /// Codex 홈 재정의(nil = `~/.codex`). 프로덕션은 로그인 셸의 `CODEX_HOME` 을 한 번 조회해 캐시한다(셸 프로세스 1회).
    package typealias CodexHomeResolver = @Sendable () async -> URL?

    package nonisolated static let snapshotKey = "check.codexAccount.snapshot"
    /// 프로브 간격(초). 계정 버킷의 반영 지연이 수십 분이라 더 자주 물어도 새 값이 없다.
    package nonisolated static let refreshInterval: TimeInterval = 1800
    ...
    package nonisolated static let forcedRefreshFloor: TimeInterval = 60

    /// 마지막으로 받은 계정 스냅샷(실패해도 직전 값을 유지 — 버킷은 날짜별이라 낡아도 틀리지 않는다).
    package private(set) var snapshot: CodexAccountUsage?
    /// 마지막 프로브의 상태. 서버 `codex_account_status` 로 올라간다.
    package private(set) var lastStatus: CodexAccountProbeStatus?
    /// 마지막 프로브 **시도** 시각(주입 시계). 간격 판정의 기준.
    package private(set) var lastProbeAt: Date?
    ...
    package static func inert() -> CodexAccountUsageStore {
...
package enum TokenUsageDisplay {
    ...
    package static func codexEffective(local: TokenUsageMonthly, account: CodexAccountUsage?) -> Int {
