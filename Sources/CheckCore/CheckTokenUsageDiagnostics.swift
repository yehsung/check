...
package struct CodexUsageDiagnostics: Codable, Equatable, Sendable {
    /// **전 기간** rollout 파일 총 개수(mtime 프리필터 없이 전량). 월 렌즈의 예외 — `filesMonth` 의 분모다.
    package var filesTotal: Int = 0
    /// 대상 월 이벤트가 하나라도 있는 파일 수.
    package var filesMonth: Int = 0
    /// 대상 월 token_count 이벤트 수.
    package var eventsMonth: Int = 0
...
package enum CodexUsageDiagnosticsScanner {
    /// month 는 KST 'YYYY-MM'. appBuild 는 결과에 그대로 실려 나간다(값의 출처 표시).
    package static func compute(homeDirectory: URL, codexHome: URL? = nil, month: String, appBuild: Int) -> CodexUsageDiagnostics {
