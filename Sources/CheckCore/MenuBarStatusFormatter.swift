...
package struct WorkStatusSnapshot: Equatable {
    package var status: WorkStatus
    package var elapsedSeconds: Int
    package var pendingSync: Bool = false

    package var isWorking: Bool {
        status == .working
    }

    package var localizedStatus: String {
        if pendingSync {
            return "동기화 대기"
        }
    ...
    package init(status: WorkStatus, elapsedSeconds: Int, pendingSync: Bool = false) {
        self.status = status
...
package enum WorkStatus: Equatable {
    case working
    case offWork
...
package enum MenuBarStatusFormatter {
    ...
    package static let maxLabelWidth: CGFloat = 82

    package static func title(for snapshot: WorkStatusSnapshot) -> String {
    ...
    package static func symbolName(for snapshot: WorkStatusSnapshot) -> String {
        if snapshot.pendingSync {
    ...
    package static func titleDuration(_ seconds: Int) -> String {
    ...
    package static func duration(_ seconds: Int) -> String {
        let safeSeconds = max(0, seconds)
        let hours = safeSeconds / 3600
        let minutes = (safeSeconds % 3600) / 60
