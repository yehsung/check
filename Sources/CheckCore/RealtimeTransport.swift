...
@MainActor
package protocol RealtimeTransport: AnyObject {
...
package enum RealtimeFeature {
    package static let defaultsKey = "check.realtimeEnabled"
    /// 개발·프로브용 탈출구. 설정을 건드리지 않고 한 번 켜 본다.
    package static let environmentName = "CHECK_REALTIME"
...
package enum RealtimeFrame {
    /// Phoenix wire 의 topic. 채널명 앞에 `realtime:` 을 붙이는 **유일한 지점**.
    package static func wireTopic(channel: String) -> String { "realtime:\(channel)" }

    /// 소켓 URL. Supabase Realtime v1 엔드포인트.
    package static func socketURL(projectURL: URL, apiKey: String) -> URL? {
    ...
    package static func join(channel: String, accessToken: String, isPrivate: Bool, ref: String) -> String {
    ...
    package static func heartbeat(ref: String) -> String {
        encode(topic: "phoenix", event: "heartbeat", payload: [:], ref: ref, joinRef: nil)
    }

    package static func accessToken(channel: String, token: String, ref: String, joinRef: String) -> String {
        encode(
    ...
    package static func decode(text: String, channel: String, joinRef: String?) -> RealtimeTransportEvent? {
    ...
    package static func classifyJoinError(reason: String) -> RealtimeJoinRejection {
...
@MainActor
package final class LiveRealtimeTransport: NSObject, RealtimeTransport {
    package var onEvent: ((RealtimeTransportEvent) -> Void)?
    ...
    package init?(session: URLSession = .shared, environment: [String: String] = ProcessInfo.processInfo.environment) {
        guard !CheckPanelVisibility.isRunningTests || environment["CHECK_E2E"] == "1" else { return nil }
        self.session = session
        super.init()
    }

    package func connect(url: URL, apiKey: String, accessToken: String, channel: String, isPrivate: Bool) {
        disconnect()
        generation &+= 1
    ...
    package func pushAccessToken(_ token: String) {
