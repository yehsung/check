...
package enum SupabaseConfig {
    package static let projectURL = URL(string: "https://xfnhfjvubetkdnfkfljg.supabase.co")!
    // 멀티팀 전환: 팀 식별자(teamID)/팀 이름(teamName) 하드코딩은 모두 제거했다. 팀 이름은 로그인 후
    // store.teamName(멤버십 조회로 확정)이 유일한 출처이고, 뷰는 이를 그대로 표시한다.
    package static let anonKeyEnvironmentName = "CHECK_SUPABASE_ANON_KEY"

    package static func anonKey(
