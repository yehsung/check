...
/// 잠그는 것은 락딜레이(0.50→0.16초)와 접지 리셋 한도다. 손실의 기전은 둘이다:
///  ① **조준보다 낮은 자리.** 위험 구간은 조각이 실제로 공중에 떠 내려오는 **중간 레벨(대략 5~14)** 이다.
///     20G(L15·advance 280 부터)는 조각이 스폰과 거의 같은 틱에 바닥에 얹혀 더 내려갈 칸이 없어 오히려 무해하다.
///  ② **접지 예산 낭비.** 바닥 근처의 한 칸은 접지 리셋 예산을 먹는데 그 한도가 레벨을 따라 15 → 2 로 줄어(`lockResetLimit`),
///     후반일수록 한 번의 낭비가 비싸다.
///
/// 즉 하드드롭에 제스처를 안 주기로 한 것과 **같은 규율**(오인식 = 조각 손실)이되 기전이 다르다. 이 판단은 폰 전용이고
/// 맥 조작과 무관하다.
