...
final class SpriteRuntime {
    ...
    private(set) var stateStartedAt: TimeInterval
    ...
    convenience init?(manifest: CharacterManifest, atlasURL: URL) {
    ...
    init?(manifest: CharacterManifest, atlas: CGImage) {
    ...
    var currentFrame: CharacterManifest.Rect {
    ...
    var atlasSize: CGSize { CGSize(width: spec.width, height: spec.height) }

    /// 상태 하나의 한 바퀴 길이(초). 없는 상태면 폴백된 상태의 길이.
    func totalDuration(of key: String) -> TimeInterval {
    ...
    func setState(_ key: String, mirrored: Bool, now: TimeInterval) {
    ...
    func isOpaque(planeUV: CGPoint) -> Bool {
