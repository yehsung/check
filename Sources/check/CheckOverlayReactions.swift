...
final class ReactionEngine {
    ...
    /// 도착 알림 말풍선을 눌렀을 때 메시지 창을 여는 클로저. **그 창이 아직 없어 지금은 언제나 nil 이다**
    /// (배선은 통합 담당). 여기에 두는 이유는 **보는 쪽과 받는 쪽이 같은 값 하나를 봐야** 하기 때문이다 —
    /// 말풍선 뷰는 이 값이 nil 인지로 화살표·버튼 성격을 정하고, 컨트롤러(`handleClick`)는 이 값으로
    /// 클릭 자리를 만들지 정한다. 둘이 갈리면 "화살표는 있는데 눌러도 아무 일이 없다"가 된다.
    /// 관찰 대상(`@Observable`)이라 배선이 꽂히는 순간 말풍선이 스스로 화살표를 얻는다.
