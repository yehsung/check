...
final class CheckOverlayController {
    ...
    /// 덮어쓸 오버라이드를 세팅하고 근무를 세운다. 이후 store 관찰 → updateWorking(true) 경로가
    ...
    ///
    /// ★ **이 자리가 "사람이 돌아왔다"의 유일한 증거 지점이다**(최근 10분 안에 실제 사용 5분). 잠에서 깨어난
    ///   복귀도 여기로 들어온다 — NudgeScheduler 는 didWake 에 활성 누적을 0 으로 되돌리므로, 깨어난 뒤
    ///   그 5분을 처음부터 다시 채워야 발화한다.
    ///
    /// ★ **새 세션을 만들기 **전에** 재개 대상을 본다. 순서가 뒤바뀌면(start 를 먼저 하면) 세션이 두 개가
    ///   되거나 재개가 조용히 죽는다 — v0.2.46 에서 지운 이어붙이기의 결함이 정확히 그 순서 문제였다.
    ///   재개 조건은 스토어가 소유한다(내 소유 · 비근무 · 사유 ≠ long_session · ended_at 30분 이내).
    func nudgeAutoStart() {
        ...
        // 반환값이 순서의 계약이다: 태울 재개가 있으면 Task 를 돌려주고, 그때는 새 세션을 만들지 않는다.
        // (재개 왕복이 실패해도 새 세션을 대신 만들지 않는다 — 다음 넛지 주기가 다시 판정하고, 그때는
        //  30분 창이 이미 닫혔을 수 있어 정상적으로 새 세션이 열린다.)
        if store.resumeRecentlyClosedSession() != nil { return }
        store.start()
    }
