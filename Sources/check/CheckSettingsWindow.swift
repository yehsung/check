...
final class CheckSettingsWindowController: NSObject, NSWindowDelegate {
    ...
    ///
    /// v0.3.13: '소속 센터' 행이 붙어 콘텐츠가 **465pt** 가 됐다(실측, preferredWidth 380 · 안내 한 줄이 제일 긴
    /// '미지정' 상태 기준). 470 을 넘지는 않지만 여유가 5pt 뿐이라 470 → 520 으로 올린다. 5pt 는 계약이 아니라
    /// 우연이다: 창을 minWidth(320)까지 좁히면 그 안내 한 줄이 두 줄로 접혀 그 자리에서 넘치고, 넘치는 순간
    /// 잘리는 것은 **맨 아래 설정 항목 하나가 통째로** 다(각주 한 줄이 아니다 — 그 각주는 2026-09-10 에 제보로 옮겼다).
    static let defaultContentSize = NSSize(width: CheckSettingsView.preferredWidth + 40, height: 520)
