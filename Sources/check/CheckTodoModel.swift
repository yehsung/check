...
struct TodoItem: Codable, Equatable, Identifiable {
    ...
    var updatedAtMs: Int64 { TodoRules.milliseconds(updatedAt) }

    /// 네 시각을 전부 정수 ms 격자에 올린 사본. 1세대 파일(소수 ms 가 남은 Date)을 읽을 때 한 번 거친다.
    func withMillisecondTimes() -> TodoItem {
        var copy = self
...
struct TodoFileSyncState: Codable, Equatable, Sendable {
...
struct TodoFile: Codable, Equatable {
    ...
    var version: Int
    var items: [TodoItem]
    var sync: TodoFileSyncState

    /// `sync` 를 안 주면 **모든 항목이 보낼 것**이다 — 동기화 상태를 모르는 파일은 1세대와 같은 규칙으로 읽는다.
    init(version: Int = TodoFile.currentVersion, items: [TodoItem], sync: TodoFileSyncState? = nil) {
...
enum TodoRules {
    ...
    static func normalizedTime(_ date: Date) -> Date {
...
@MainActor @Observable final class TodoListStore {
    ...
    @ObservationIgnored var syncProtectedIDs: @MainActor () -> Set<UUID> = { [] }
    ...
    func makeSyncRequest(ids: [UUID]) -> TodoSyncOutgoing {
    ...
    @discardableResult
    func applySync(_ result: TodoSyncResult, for outgoing: TodoSyncOutgoing) -> Bool {
