...
extension SupabaseWorkService {
    ...
    func fetchMessageHistoryWithReads(
        accessToken: String,
        hours: Int,
        limit: Int
    ...
    func fetchMessageUnreadSummary(accessToken: String) async throws -> MessageUnreadSummaryResponse {
        let data = try await send(
    ...
    nonisolated static let messageHistoryWithReadsPath = "/rest/v1/rpc/message_history_with_reads"
    nonisolated static let messageUnreadSummaryPath = "/rest/v1/rpc/message_unread_summary"
    nonisolated static let markMessagesReadPath = "/rest/v1/rpc/mark_messages_read"
    ...
    func messageHistoryEntry(fromReadsRow readsRow: MessageHistoryReadsRow) -> MessageHistoryEntry? {
...
struct MessageHistoryReadsRow: Decodable, Equatable, Sendable {
    let id: String?
    let fromUser: String?
    let toUser: String?
    let body: String?
...
struct MessageReadMarkRequest: Encodable, Equatable, Sendable {
    let pPeer: String
    let pThrough: String?
}

/// `mark_messages_read` 응답. `{"status":"ok","advanced":bool,"unread":int}` · unauthorized · invalid.
struct MessageReadMarkResponse: Decodable, Equatable, Sendable {
    let status: String?
    let advanced: Bool?
    let unread: Int?
...
struct MessageUnreadSummary: Equatable, Sendable {
    let total: Int
    let peers: [MessageUnreadPeer]

    var unreadPeerIDs: Set<String> { Set(peers.map(\.peerUserID)) }
}

struct MessageUnreadPeer: Equatable, Sendable {
    let peerUserID: String
    let count: Int
