...
final class WorkTimerStore {
    ...
    func setMenuPresented(_ presented: Bool) {
        ...
        if presented {
            ...
            // 운영자에게만: 미해결 **신고** 건수를 여는 순간 한 번 묻는다(v0.3.34). 위 답장 배지와 같은 자리·같은 빈도이고
            // 타이머는 없다. 심사에 "접수한 신고는 24시간 안에 확인합니다"라고 약속했는데 신고가 오는 곳은 폰이라,
            // 운영자가 [제보] 화면을 열 때까지 기다리면 그 약속이 운영자의 기억에 매인다 — 팝오버를 여는 김에 물어 두면
            // 레일 배지와 메뉴바 점이 그다음부터 운영자를 붙잡는다. 비운영자는 묻지도 않는다(가드는 그 함수 안).
    ...
    /// 루비 잔량. **nil = 아직 모름**(한 번도 못 읽었거나 실패).
    ///
    /// ★ **0 으로 두지 마라.** 0 은 "모른다"가 아니라 **"없다"** 로 읽힌다 — 실제 잔량이 3인 사용자에게
    ///   화면이 "0"이라고 말한 신고가 이미 있었다(2026-09-13, 헤더 칩). `ultraBalance` 가 같은 이유로
    ///   Optional 인 것과 같은 규약이고, 화면은 nil 을 "—" 로 그린다.
    ///
    /// **서버가 유일한 출처다** — 구매 성공 응답이 실어 준 값으로만 갱신하고, 클라가 스스로 빼지 않는다.
    /// 두 곳에서 빼면 실패한 구매가 화면에서만 차감되는 조합이 생긴다.
    var rubyBalance: Int?
    /// 한 번에 살 수 있는 울트라 최대 개수(서버 상수). 지금 화면은 1개 고정이라 판정에 안 쓴다.
    var ultraBuyMax: Int?
    ...
    /// 한 번이라도 받았는가(빈 목록과 로드 전을 가른다 — 제보 목록과 같은 3플래그 규약). 스키마 부재도 여기서 true.
