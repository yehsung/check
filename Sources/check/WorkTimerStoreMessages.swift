...
extension WorkTimerStore {
    ...
    nonisolated static let messageHistoryHours = MessageNoticeText.historyHours
    /// 한 번에 받아 올 이력 건수. 서버가 1~500 으로 접는다. 26명 규모에서 하루 대화가 이 수를 넘기 어렵고,
    /// 넘으면 **오래된 쪽이 빠진다**(서버가 최신부터 자른다) — 그게 사라지는 규칙과 같은 방향이라 어색하지 않다.
    nonisolated static let messageHistoryLimit = MessageNoticeText.historyLimit
    ...
    //
    // D-base: 문장 자체는 코어 `MessageNoticeText` 로 옮겼다(폰 메시지 탭이 같은 문장을 쓴다). 아래 상수는 그 전달이고, 뜻의 설명은 여기 남는다.
    //
    // **전송 결과를 쓰는 곳은 `messageNotice` 하나뿐**이므로 문구도 여기 산다 — 뷰가 자기 표를 따로 들면
    // 그중 한 벌은 반드시 낡는다(ultraSpentNotice 를 상수에서 파생시킨 것과 같은 규약).

    nonisolated static let messageSentNotice = MessageNoticeText.sent
    nonisolated static let messageNotWorkingNotice = MessageNoticeText.notWorking
    ...
    nonisolated static let messageTargetNotWorkingNotice = MessageNoticeText.targetNotWorking
    ...
    nonisolated static let messageTargetFocusedNotice = MessageNoticeText.targetFocused
    /// 서버가 텍스트 난간에 걸었을 때. **길이 문구와 합치면 안 된다** — 못 보내는 이유가 길이라고 읽으면
    /// 사용자는 글자를 줄이고, 줄여도 계속 막힌다(3글자 시절 이모지 안내가 갈라져 있던 것과 같은 근거).
    nonisolated static let messageNotTextNotice = MessageNoticeText.notText
    /// 서버가 메시지 기능을 통째로 내려 둔 구간(점검·사고 대응). "다시 시도"를 말하지 않는다 —
    /// 우리가 올릴 때까지 안 풀리는 상태라, 재시도를 권하면 같은 실패를 반복하게 만든다(제보의 스키마 부재와 같은 판단).
    nonisolated static let messageBlackoutNotice = MessageNoticeText.blackout
    /// 우리가 모르는 실패. **`flood` 도 여기로 접힌다**(위 머리 주석 ③) — 숫자도 카운트다운도 없는 한 문장이다.
    nonisolated static let messageInvalidNotice = MessageNoticeText.invalid
...
// D-base: 순수 규칙(읽음 값 타입 · `MessageUnreadRules` · 정렬 · `MessageThreadBuilder`)과 안내 문장은 코어 `MessageRules.swift` 로 옮겼다.
