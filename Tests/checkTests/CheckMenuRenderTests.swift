...
///
/// ⚠️ **바이트 비교로 되돌리지 마라.** `#expect(a == b)` 에 29만 바이트짜리 Data 두 개를 주면 통과할 땐
/// 싸지만 **실패하는 순간** swift-testing 이 두 컬렉션의 차분을 계산하느라 프로세스가 코어를 물고 사실상
/// 끝나지 않는다 — 스위트 전체가 멎고 `Killing swiftpm-testing` 으로 죽는다. 픽셀은 폰트 래스터화·타이밍으로
/// 가끔 흔들리므로 코드가 멀쩡해도 그 상황이 온다. 해시로 비교하면 실패해도 64글자 두 개만 찍고 즉시 빨개진다.
...
@MainActor
@Test
func autoClosedSessionNoLongerRendersARecoveryBanner() throws {
    // v0.2.47 — 자리 비움 [되돌리기] 인라인 배너는 제거됐다. 구제는 배너·버튼이 아니라 복귀 감지 시점의
    // **자동 재개**(30분 창)이고, 그래서 자동 마감 직후의 팝오버는 평소와 **한 픽셀도 다르지 않아야** 한다.
    // 이 단언이 없으면 배너를 되살려도(또는 되살아나도) 아무도 모른다.
    let now = Date()

    // 자동 마감 직후 상태 그대로 — 재개 대상이 서 있다(창 안, 사유 abandoned, 비근무).
    let closedStore = makeTeamStore(members: presenceMembers(now: now), now: now)
    closedStore.lastAutoClosedSessionID = "11111111-2222-3333-4444-555555555555"
    closedStore.lastAutoClosedStartedAt = now.addingTimeInterval(-7_200)
    closedStore.lastAutoClosedEndedAt = now.addingTimeInterval(-30)
    closedStore.lastAutoClosedReason = .abandoned
    closedStore.lastAutoClosedAt = now.addingTimeInterval(-30)
    closedStore.syncMessage = "자리 비움으로 자동 근무종료됨"
    // 전제: 재개 대상은 실제로 살아 있다(아래 '창이 안 자란다'가 공허해지지 않게 못 박는다).
    #expect(closedStore.canResumeRecentlyClosedSession(now: now))

    let closedPNG = try renderPNG(CheckMenuView(store: closedStore))
    #expect(closedPNG.count > 0)
    saveV0211Snapshot(closedPNG, "auto-close-no-banner")

    // 재개 대상이 있어도 창은 자라지 않는다(= 배너가 그려지지 않는다).
    let plain = try #require(renderedPixelHeight(CheckMenuView(store: makeTeamStore(members: presenceMembers(now: now), now: now))))
    let withTarget = try #require(renderedPixelHeight(CheckMenuView(store: closedStore)))
    #expect(withTarget == plain, "자동 마감 대상이 팝오버 높이를 \((withTarget - plain) / 2)pt 바꿨습니다 — 배너가 되살아났습니다")
    #expect(Double(withTarget) / 2.0 <= 700.0)

    // 대조군: 회고 배너는 그대로 살아 있다(인라인 배너 자체를 지운 게 아니라 되돌리기 배너만 지웠다).
    let retroStore = makeTeamStore(members: presenceMembers(now: now), now: now)
    retroStore.retro = sampleWeeklyRetro()
    retroStore.showsRetroBanner = true
    let withRetro = try #require(renderedPixelHeight(CheckMenuView(store: retroStore)))
    #expect(withRetro > plain)
}
...
func insightsOverflowFadeMarksTheClippedBottomOfTheBody() throws {
    ...
    //     2026-09-22 재정렬(「지난주」 → 「최근 12주」) 뒤로 접힘선에 걸리는 것은 **근무 잔디**다(토큰 잔디는 통째로
    //     접힘선 아래로 내려갔다 — InsightsPanel 머리 주석). 픽스처 1열(week 1)의 월~목이 1/2/3/4단계라 잘린 행이
    //     가장 진한 칸이고, 그라데이션이 빠지면 바로 그 칸이 접힘선까지 칸 색으로 닿는다.
    let store = makeInsightsStore()
    store.dailyGrid = sampleWorkDailyGrid()
    ...
    // **열을 하나로 못 박지 않는다**(2026-09-22): 재정렬로 접힘선에 걸리는 격자가 토큰 잔디 → 근무 잔디로 바뀌었고,
    // 그 격자의 1열 월요일은 픽스처에서 빈 칸이라 그 열에는 잘린 칸이 없다(그 열만 보면 마지막 칠해진 칸이 한참 위
    // 히트맵이라 gap 이 142px 로 벌어진다 — 옛 accent 전용 시절과 같은 함정을 픽스처 좌표로 다시 밟은 셈이다).
    // 그래서 앞 네 열을 훑어 **잘린 칸이 실제로 있는 열**(gap 이 가장 작은 열)에서 잰다.
    let columns = [72, 90, 108, 126].map { $0 * 2 }
    let gaps = columns.compactMap { gapBetweenLastAccentAndPanelRun(clipped, x: $0) }
    let gap = try #require(gaps.min(), "잔디 칸 아래에 패널색 구간이 있어야 한다")
    #expect(gap >= 8, "잘린 행이 그라데이션 없이 접힘선까지 칸 색으로 닿았다(gap \(gap)px)")
    #expect(gap <= Int(InsightsOverflowFade.height) * 2 + 4, "잘린 칸이 그라데이션 없이 접힘선까지 닿았다(gap \(gap)px)")
}
...
func insightsPanelShowsTheGrassEvenWhenLastWeekWasEmpty() throws {
    ...
    // 이 본문은 회고 카드가 빈 줄 하나라 97pt 짧다 — 그 차이는 토큰 잔디 유무와 무관하게 같다(재정렬 뒤 실측 680.5−583.5 = 499−402).
    ...
    // 토큰 잔디까지 있는 짧은 본문(583.5pt)은 여전히 상한을 넘겨 425pt 로 깎이고 창은 695pt 에 선다.
    ...
    // 수집 거부자는 토큰 섹션이 빠져 본문이 181.5pt 더 짧다(402pt) — 크롬이 없으면 깎지 않아 스크롤도, 바닥의 빈 띠도 없다.
    // (큰 본문 기준 425pt 로 못 박으면 402pt 본문 아래 23pt 가 늘 비어 보인다.) 이 상태의 창은 실측 672pt.
    let shortNoToken = InsightsPanelChromeBudget.naturalHeight(hasRetro: false, showsTokenGrid: false)
    #expect(shortNoToken == 402)
    ...
    #expect(Double(scrollHeight) / 2.0 == 672)
    // 목표 편집 행(92)이 얹히면 늘어난 여유(+23)를 넘긴 69pt 만 깎아 창은 다시 695pt 에 선다.
    #expect(InsightsPanelChromeBudget.capHeight(extraChromeHeight: CheckMenuView.goalEditorHeight, naturalHeight: shortNoToken) == shortNoToken - 69)
...
func popoverStaysWithinHeightCapForWorstBannerAndPanelCombinations() throws {
    ...
    // (a) 홈: 토큰 행 + 회고 배너 + 새 버전 배너(가장 많이 겹치는 상황).
    let home = teamStore()
    addRetroBanner(home)
    ...
    // (f) 개인 기록 패널(회고 카드 + 7×24 히트맵) + 회고/새 버전 배너.
    ...
    addRetroBanner(insights)
    try measure("insights+banners", CheckMenuView(store: insights, previewUpdateBanner: true, previewUpdateNotes: sampleUpdateNotes))

    // (g) 하위 패널 + 인라인 배너. 배너는 홈에만 있는 것이 아니다 — 헤더 카드 아래 형제라
    //     리그/토큰/찌르기 화면 위에도 그대로 얹힌다.
    let bannerPanel = teamStore()
    addRetroBanner(bannerPanel)
    bannerPanel.leaderboard = manyLeaderboardEntries(count: 12)
...
func onlyOneBannerIsDrawnAtATime() throws {
    ...
    // 회고 + 새 버전 두 후보가 모두 자격을 갖춰도 실제로 그려지는 건 하나뿐이라
    // 창 높이는 '가장 급한 배너 하나'만큼만 늘어난다(회고가 업데이트를 이긴다).
...
func menuBodyDoesNotObserveDisplayNowOnHomeScreen() {
    // 회귀 지점: topBanner 가 canResumeRecentlyClosedSession(now: store.displayNow) 를 직접 불러, 배너가 하나도 없는
    ...
    // 반대로 배너 상태가 실제로 바뀌면(회고 배너 등장) body 는 반드시 다시 그려져야 한다 —
...
func mainMenuPokeIconSurfacesRealtimeFailureAtZeroHeightCost() throws {
    ...
    // 대조군: 같은 입력이면 바이트까지 같다(해시 비교 — 판정력은 같고 실패가 스위트를 멈추지 않는다.
    // 이유는 pngDigest 주석에 있다).
    #expect(pngDigest(healthyPNG) == pngDigest(twinPNG))
    #expect(pngDigest(healthyPNG) != pngDigest(failingPNG), "리얼타임 실패가 메인 메뉴에 아무 자국도 안 남겼다 — 하위 화면을 안 여는 사람은 영영 모른다.")
...
// MARK: - 메뉴바 라벨 (상태 + 동기화 대기만 말한다)
//
// v0.2.46 에서 '이어붙이기'가 사라지면서 메뉴바가 말하는 것은 둘로 줄었다: 지금 근무 중인가(시계)와
// 아직 서버에 못 올린 조작이 있는가(대기). 아래 테스트는 그 둘이 **픽셀로도** 갈리는지와,
// 하루 종일 떠 있는 라벨이 폭 예산 안에 있는지를 잰다.

@Test
func menuBarLabelSpeaksOnlyStatusAndPendingSync() {
    let off = WorkStatusSnapshot(status: .offWork, elapsedSeconds: 0)
    #expect(MenuBarStatusFormatter.title(for: off) == "오프")
    #expect(MenuBarStatusFormatter.symbolName(for: off) == "pause.circle.fill")

    // 제목은 항상 시:분이다(v0.2.43 — 초를 보이면 티커를 1초로 묶는다).
    let working = WorkStatusSnapshot(status: .working, elapsedSeconds: 84)
    #expect(MenuBarStatusFormatter.title(for: working) == "00:01")
    #expect(MenuBarStatusFormatter.symbolName(for: working) == "figure.run.circle.fill")

    // 동기화 대기는 근무/비근무 양쪽에서 이긴다 — 아직 서버에 없는 조작이 시계보다 급하다.
    ...
    let pendingOff = WorkStatusSnapshot(status: .offWork, elapsedSeconds: 0, pendingSync: true)
    #expect(MenuBarStatusFormatter.title(for: pendingOff) == "대기")
    #expect(MenuBarStatusFormatter.symbolName(for: pendingOff) == "exclamationmark.icloud.fill")
}

/// 라벨 글자는 **스토어의 파생 저장값(menuBarTitle)을 한 글자도 바꾸지 않고** 그린다.
/// (v0.2.45 까지는 라벨이 displayTitle 로 저장값을 다시 가공했다 — 복원 표식을 얹기 위해서였고,
///  그 기능이 사라진 지금 가공이 남으면 시계가 옛 값에 얼어붙는 경로만 되살아난다.)
@MainActor
@Test
func menuBarLabelDrawsTheStoredTitleVerbatim() throws {
    func text(_ snapshot: WorkStatusSnapshot, stored: String) throws -> NSBitmapImageRep {
    ...
    // ★ 스냅샷이 만들어 낼 글자("01:01", 5칸)보다 **짧은** 저장값을 넣는다. 라벨이 저장값을 그대로
    //   그리면 폭이 줄고, 스냅샷에서 다시 만들면 폭이 그대로다 — 폭은 픽셀 비교와 달리 결정적이라
    //   렌더의 안티에일리어싱 흔들림에 흔들리지 않는다.
    let working = WorkStatusSnapshot(status: .working, elapsedSeconds: 3_661)
    #expect(MenuBarStatusFormatter.title(for: working) == "01:01", "픽스처: 스냅샷이 5칸 시계를 만든다")
    let derived = try text(working, stored: "01:01")
    let shorter = try text(working, stored: "1:1")
    #expect(
        shorter.pixelsWide < derived.pixelsWide,
        "라벨이 저장값 대신 스냅샷에서 글자를 다시 만들었습니다(저장값이 짧아졌는데 폭이 그대로입니다)"
    )

    // 반대 방향도 본다: 저장값이 길어지면 라벨도 넓어진다(글자를 통째로 무시하지 않는다).
    let longer = try text(working, stored: "001:001")
    #expect(longer.pixelsWide > derived.pixelsWide)
}

/// 메뉴바 라벨은 (a) 바 높이(22pt) 안에 들어가고, (b) 폭 예산을 넘지 않으며,
/// (c) 동기화 대기가 평소 근무와 **픽셀로 다르다**. (c)가 없으면 문자열만 바뀌고 화면은 그대로인
/// (마스코트가 심볼을 덮는) 상태를 이 테스트가 초록으로 통과시킨다.
@MainActor
@Test
func menuBarLabelStaysInsideItsWidthBudgetAndPendingSyncLooksDifferent() throws {
    ...
    // 최장 라벨: 근무 중 "23:59"(제목은 항상 HH:MM 이라 이것이 상한이다).
    let widest = WorkStatusSnapshot(status: .working, elapsedSeconds: 86_340)
    let widestBitmap = try render(widest, stored: MenuBarStatusFormatter.title(for: widest))
    let width = Double(widestBitmap.pixelsWide) / 2.0

    // (a) 바 높이. 캐릭터/심볼이 잘리면 아이콘이 사라진 것처럼 보인다.
    #expect(Double(widestBitmap.pixelsHigh) / 2.0 <= 22.5)
    // (b) 폭 예산(메뉴바는 다른 앱과 나눠 쓰는 공간이다).
    #expect(
        width <= Double(MenuBarStatusFormatter.maxLabelWidth),
        "메뉴바 라벨이 폭 예산을 넘었습니다: \(width)pt > \(MenuBarStatusFormatter.maxLabelWidth)pt"
    )
    // 예산이 실측보다 터무니없이 헐거우면 아무것도 못 막는다 — 여유는 두되 상한이 살아 있게 둔다.
    #expect(width >= Double(MenuBarStatusFormatter.maxLabelWidth) - 12)

    // (c) 동기화 대기는 같은 근무 상태에서도 다른 그림이어야 한다(글자 + 아이콘 양쪽).
    let pending = WorkStatusSnapshot(status: .working, elapsedSeconds: 86_340, pendingSync: true)
    let pendingBitmap = try render(pending, stored: "대기")
    let iconBand = min(widestBitmap.pixelsWide, pendingBitmap.pixelsWide) / 3
    #expect(
        bandDiffers(widestBitmap, pendingBitmap, width: iconBand),
        "동기화 대기인데 메뉴바 아이콘 자리가 평소와 같은 픽셀입니다(마스코트가 심볼을 덮었습니다)"
    )
    #expect(widestBitmap.pixelsWide != pendingBitmap.pixelsWide)
}

// MARK: - 이어붙이기 제거 회귀 그물 (v0.2.46) — 팝오버 반쪽

/// 서버 `away_sync()` 는 `restorable` 을 **계속 실어 보낸다**(docs/away-close.md 5절 — RPC 도 휴면 중).
/// 그 응답이 실제 디코드 경로(`awaySync(from:)` → `applyAwaySync`)를 지난 뒤에도 팝오버는 배너를
/// 한 픽셀도 더 그리지 않아야 한다. 스토어 쪽 반쪽은
/// AwayCloseTests 의 serverResumeHintLeavesNoTraceInTheStore 다.
@MainActor
@Test
func popoverDrawsNoBannerWhenTheServerStillOffersToResume() async throws {
    let now = Date()
    let base = makeTeamStore(members: steadyMembers(count: 4), now: now, tokenUsage: seededTokenStore())
    let served = makeTeamStore(members: steadyMembers(count: 4), now: now, tokenUsage: seededTokenStore())

    // docs/away-close.md 2절 모양 그대로. 창은 활짝 열려 있다(잔여 5시간) — 옛 클라라면 배너가 떴을 상태다.
    let formatter = ISO8601DateFormatter()
    let closedAt = now.addingTimeInterval(-9_000)
    let json = """
    {"status":"ok","serverNow":"\(formatter.string(from: now))",
     "closeThresholdSeconds":9000,"backstopSeconds":10800,"freezeSeconds":1800,
     "restoreWindowSeconds":21600,"dailyRestoreLimit":2,
     "restorableReasons":["away","sleep"],"restoresUsedToday":0,"restoresLeftToday":2,
     "openSession":null,
     "restorable":{"sessionId":"aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee",
       "teamId":"\(URLProtocolStub.stubTeamID)",
       "startedAt":"\(formatter.string(from: closedAt.addingTimeInterval(-11_400)))",
       "endedAt":"\(formatter.string(from: closedAt))",
       "durationSeconds":11400,
       "autoClosedAt":"\(formatter.string(from: closedAt))",
       "autoClosedReason":"away",
       "expiresAt":"\(formatter.string(from: closedAt.addingTimeInterval(21_600)))",
    ...
    let service = SupabaseWorkService(
        projectURL: URL(string: "http://restorable-ignored")!,
        anonKey: "anon-test-key",
        session: URLSession(configuration: .stubbed)
    )
    served.applyAwaySync(await service.awaySync(from: response))

    // 대조군: 응답 자체는 확실히 소비됐다(정책이 섰다) — 통째로 버려서 초록이 된 것이 아니다.
    #expect(served.awayServerSupported)
    #expect(served.awayPolicy?.closeThresholdSeconds == 9_000)

    let plain = try #require(renderedPixelHeight(CheckMenuView(store: base)))
    let after = try #require(renderedPixelHeight(CheckMenuView(store: served)))
    #expect(after == plain, "서버가 보낸 restorable 로 팝오버가 \((after - plain) / 2)pt 자랐습니다(배너가 되살아났습니다)")

    // 높이만 보면 "배너가 떴는데 목록이 한 행 줄어 상쇄됐다"를 놓친다. 배너가 실제로 더하는 잉크
    //   (pending 주황 틴트 + 테두리 + 버튼 캡슐)를 함께 센다 — 옛 배너는 이 값을 3,000픽셀 넘게 올렸다.
    //   완전 동일(0)을 요구하지는 않는다: 같은 화면을 두 번 그려도 안티에일리어싱이 몇 픽셀 흔들린다.
    ...
    let servedBitmap = try renderBitmap(CheckMenuView(store: served))
    let addedInk = orangeInk(servedBitmap) - orangeInk(plainBitmap)
    #expect(abs(addedInk) < 300, "서버가 보낸 restorable 이 팝오버에 주황 잉크 \(addedInk)픽셀을 더했습니다(배너가 되살아났습니다)")
}
...
/// 이것이 배너 높이 상수가 실제로 하는 일이다(CheckMenuView.listExtraChromeHeight → ListRowBudget).
/// 상수가 0 이거나 실제보다 작으면 목록이 한 행 더 남아 창이 700pt 를 넘고, 그 순간
/// 푸터(로그아웃/앱 종료)가 화면 밖으로 나간다.
@MainActor
@Test
func inlineBannerBuysItsHeightFromTheTeamListInsteadOfTheWindow() throws {
    let now = Date()
    ...
    banner.retro = sampleWeeklyRetro()
    banner.showsRetroBanner = true
...
func checkMenuViewRendersInsightsTokenGridSnapshot() throws {
    ...
    // ★ 2026-09-22 재정렬 뒤로 토큰 잔디는 **접힘선 아래**다(「지난주」 묶음이 앞으로 왔다 — InsightsPanel 머리 주석).
    //   그래서 기본 스토어의 클립 렌더에는 토큰 격자가 한 줄도 안 나온다. 값→픽셀 배선을 그 화면에서 재면
    //   "값을 바꿔도 픽셀이 그대로"가 되어 이 테스트가 배선 결함과 구분되지 않는다(재정렬 때 실제로 그렇게 빨개졌다).
    //   그래서 **본문이 안 깎이는 가장 짧은 상태**에서 잰다: 지난주가 비어 회고·히트맵이 짧은 휴가 복귀 주.
    //   (hasTokenGrass 가 true 라 자리 문구가 본문을 덮지 않는다.)
    //   그래서 패널을 **자연 높이로 직접** 그린다(extraChromeHeight 를 크게 음수로 주면 capHeight 가 nil 이라 안 깎인다).
    //   스토어 → 뷰 배선(`tokenDailyGrid: store.tokenDailyGrid`)은 이 파일의 다른 소스 계약이 따로 못 박는다.
    func panel(_ token: TokenDailyGrid) -> some View {
        InsightsPanel(
            heatmap: sampleWorkRhythmHeatmap(),
            retro: sampleWeeklyRetro(),
            dailyGrid: sampleWorkDailyGrid(),
            tokenDailyGrid: token,
            showsTokenGrid: true,
            hasLoaded: true,
            extraChromeHeight: -5000,
            onBack: {}
        )
        .frame(width: 316)
        .fixedSize()
    }
    let tokenGrid = sampleTokenDailyGrid()
    #expect(tokenGrid.totalTokens > 0)
    let painted = try renderBitmap(panel(tokenGrid))
...
func insightsPanelHidesTheTokenGrassWhenTokenCollectionIsOptedOut() throws {
    ...
    // 없으면 자연 높이 그대로 선다(672pt). 픽셀 차이는 "섹션이 있다/없다"를 이만큼 분명히 말하지 못한다.
    ...
    #expect(Double(withoutSection) / 2.0 == 672)
...
@MainActor
@Test
func insightsPanelGroupsLastWeekThenTheTwelveWeekGrasses() throws {
    // 2026-09-22 사용자 지시로 v0.2.41 순서를 뒤집었다 — "지난주 회고 밑에 12주 잔디가 뜨고 그 밑에 지난주 근무
    // 리듬이 뜨는 게 이상하다". 회고와 히트맵은 같은 주 창에서 나와 총합이 같은 숫자라 한 묶음이어야 한다.
    // 묶음 경계는 **시간 범위**다: 「지난주」(회고 → 히트맵) → 「최근 12주」(근무 잔디 → 토큰 잔디). 폰도 같은 기준.
    let source = swiftCodeStrippingComments(try String(contentsOf: checkMenuViewSourceURL(), encoding: .utf8))
    let panel = try #require(swiftStructBody(source, name: "InsightsPanel"))
    ...
    let order = try ["retroCard", "heatmapSection", "dailyGridSection", "tokenGridSection"].map {
        try #require(content.range(of: $0)?.lowerBound, "본문에 \($0) 이 없다")
    }
    #expect(order == order.sorted(), "본문 섹션 순서가 회고 → 히트맵 → 근무 잔디 → 토큰 잔디 가 아니다")
    ...
    // 묶음 **경계**의 구분선만 여백을 더 받는다(묶음 안 구분선은 여백 0). 경계는 히트맵과 근무 잔디 사이 하나뿐이다.
    #expect(content.components(separatedBy: "InsightsSectionGroupGap.extraSpacing").count - 1 == 1,
            "묶음 경계 여백이 없거나 두 곳 이상에 붙었다 — 경계는 「지난주」와 「최근 12주」 사이 하나다")
    let boundary = try #require(content.range(of: "InsightsSectionGroupGap.extraSpacing")?.lowerBound)
    let heatmapAt = try #require(content.range(of: "heatmapSection")?.lowerBound)
    let dailyAt = try #require(content.range(of: "dailyGridSection")?.lowerBound)
    #expect(heatmapAt < boundary && boundary < dailyAt, "묶음 경계 여백이 히트맵과 근무 잔디 사이에 있지 않다")
...
func insightsPanelChromeBudgetMatchesTheMeasuredFourBodyCombinations() throws {
    ...
    #expect(InsightsPanelChromeBudget.naturalHeight(hasRetro: true, showsTokenGrid: true) == 680.5)
    #expect(InsightsPanelChromeBudget.naturalHeight(hasRetro: false, showsTokenGrid: true) == 583.5)
    #expect(InsightsPanelChromeBudget.naturalHeight(hasRetro: true, showsTokenGrid: false) == 499)
    #expect(InsightsPanelChromeBudget.naturalHeight(hasRetro: false, showsTokenGrid: false) == 402)
    ...
    // 크롬이 없어도 세 조합은 깎여 본문이 425pt 로 수렴하고(창 695pt), 가장 짧은 조합만 자연 높이로 선다(창 672pt).
    #expect(InsightsPanelChromeBudget.capHeight(extraChromeHeight: 0, naturalHeight: 680.5) == 425)
    #expect(InsightsPanelChromeBudget.capHeight(extraChromeHeight: 0, naturalHeight: 583.5) == 425)
    #expect(InsightsPanelChromeBudget.capHeight(extraChromeHeight: 0, naturalHeight: 499) == 425)
    #expect(InsightsPanelChromeBudget.capHeight(extraChromeHeight: 0, naturalHeight: 402) == nil)

    // 실측으로 되묻는다 — 상수만 고치고 레이아웃을 안 고쳐도(또는 그 반대) 여기서 갈린다.
    for (hasRetro, showsToken, expected) in [(true, true, 695.0), (false, true, 695.0), (true, false, 695.0), (false, false, 672.0)] {
