...
//
// v0.2.51 부터 버튼은 2단이다(1단 3시간 · 2단 계속 — 사용자 지시 2026-09-11). 서버 계약은 그대로(불리언 + PATCH)이고,
// 단계·만료·재시작·계정 교체·기존 사용자 규칙은 V0251FocusStageTests 가 검증한다. 이 파일은 옛 진입점이
// 새 순환으로 **위임되는지**와 서버 응답 매핑만 지킨다.
//
// ⚠️ 앞 판 스펙의 "기존 사용자 전환이 한 번만 일어난다" 테스트는 이 저장소에 들어온 적이 없고, 그 규칙 자체가
//    폐기됐다(2026-09-11 사용자 지시 — 기록 없는 기존 사용자는 2단으로 둔다). 되살리지 마라.
...
/// `swift test --filter FocusMode` 가 이 파일 전체를 잡도록 스위트로 묶는다(전역 함수 id 는 소문자 `focusMode…` 라
/// 필터에서 빠진다 — 검증 명령이 조용히 절반만 돌리는 일을 막는다).
@Suite struct FocusModeTests {
    ...
    func toggleFocusModeAppliesOptimisticallyAndPatchesProfile() async {
        ...
        // v0.2.51: 옛 토글은 순환(cycleFocusStage)으로 **위임**된다 — 한 번 누르면 1단이다.
        // 옛 뜻(기록 없이 켜기)으로 남으면 새 버튼 배선 전의 옛 버튼이 곧바로 2단(무기한)을 켠다.
        #expect(store.focusStage(now: store.clock()) == .timed)
        #expect(store.focusStageRecord(for: "00000000-0000-0000-0000-000000000002")?.stage == .timed)
