...
// MARK: - 넛지 자동 시작이 입력 사슬에 손대지 않는다
//
// 이 파일의 세 사고는 전부 "창을 만지는 새 기능이 입력 사슬의 값 하나를 굳혔다"였고, v0.2.32 의
// 드래그 사망도 정확히 이 계층에서 났다. 넛지 자동 시작은 말풍선 오버라이드를 세우고 근무를 시작해
// 패널을 띄우는 **창을 만지는 경로**라, 그 뒤에도
//
...
@MainActor
@Test
func nudgeAutoStartLeavesTheInputChainUntouched() {
    @MainActor
    ...
    // ① 못 박기는 시작 시점에도, 넛지가 지나간 뒤에도 서지 않는다(v0.2.32 의 드래그 사망이 이 값이었다).
    #expect(gates(rig).pinned == nil, "픽스처: 시작 시점에 못 박기가 없어야 한다")
    rig.controller.nudgeAutoStart()
    #expect(gates(rig).pinned == nil, "넛지 자동 시작이 클릭 통과를 못 박았다")

    // 픽스처가 실제로 그 경로를 탔는가(아무 일도 안 해서 통과하지 않게).
    #expect(rig.store.startedAt != nil, "픽스처: 넛지가 근무를 시작하지 않았다")

    // 자동 시작이 스토어를 근무중으로 바꿨다 — 프로덕션과 같은 순서로 컨트롤러에 흘린다.
    rig.controller.updateWorking(true)
    rig.layout()
    #expect(gates(rig).pinned == nil, "넛지 뒤 표시 경로가 클릭 통과를 못 박았다")

    // ② 값이 아니라 결과. 자동 시작으로 등장한 캐릭터도 끌 수 있어야 한다.
    #expect(rig.dragMovesPanel(), "넛지 자동 시작 뒤 드래그가 죽었다")
