...
// MARK: - 넛지 자동 시작 — 물어보지 않고 시작하되, 시작했다고 **말은 한다**
//
...
@MainActor
@Test
func autoStartAnnouncesItselfAndStartsWork() {
    // 자동 시작은 끌 수 없는 기본 동작이라 **조용히** 열면 "왜 저절로 시작됐지?"가 남는다.
    // 등장 말풍선 한 줄이 그 침묵을 깨는 유일한 채널이다.
...
@MainActor
@Test
func autoStartDoesNotStrandABubbleWhenTheCharacterIsHidden() {
    // 캐릭터를 끈 사용자에게는 말풍선 채널이 없다. 그래도 (1) 근무는 시작돼야 하고,
    // (2) 오버라이드를 세워 두면 안 된다 — 몇 시간 뒤 캐릭터를 다시 켜는 순간 낡은 안내가 튀어나온다.
