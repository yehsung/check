...
func boardDetailTooltipKeepsTheMyBoxVocabularyAndFullPrecision() {
    // 내 박스 툴팁(TokenUsageMonthly.detailTooltip)과 같은 모양·같은 정밀도(grouped): 캡션에 보이는 두 값의 정확한 숫자만.
    // v0.2.45: 그 외(입력·출력·캐시 내역, 로컬/계정 구분, "총합은 계정 집계 기준", 포크 의심, 하루 축)는 운영자 진단이라 UI 에서 걷어냈다.
    ...
    // Codex 는 codexEffective(옛 RPC 픽스처라 max(로컬 120, 계정 1,000) = 1,000) — 캡션·굵은 총합과 같은 값.
    #expect(full.detailTooltip == "Claude 10 · Codex 1,000")
    let localWins = toolMixEntry(claudeInput: 10, codexInput: 120, codexCacheRead: 60, codexAccountMonth: 120)
    #expect(localWins.detailTooltip == "Claude 10 · Codex 120")
    // 값이 0 인 소스는 통째로 뺀다(기존 관용구와 동일).
    #expect(toolMixEntry(codexInput: 120, codexCacheRead: 60).detailTooltip == "Codex 120")
    #expect(toolMixEntry(claudeInput: 10).detailTooltip == "Claude 10")
    ...
    #expect(big.detailTooltip == "Claude 19,658,964,272 · Codex 2,543,110")
    // 로컬이 0 인데 계정 집계만 있는 사람(기기가 아직 못 읽었지만 계정에는 잡힌다) — 유효값(계정)만 남는다.
    let accountOnly = toolMixEntry(codexAccountMonth: 300_000_000)
    #expect(accountOnly.detailTooltip == "Codex 300,000,000")
    #expect(accountOnly.toolUsageLabel == "Codex 3억")
    // 진단 어휘 부정 단언.
    for tip in [full, localWins, big, accountOnly].map(\.detailTooltip) {
        for word in ["로컬", "계정 집계", "캐시", "포크", "축", "총합", "기준", "입력", "출력"] {
