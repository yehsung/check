...
func migrationContractMiniGamePrize() throws {
    ...
    #expect(body.contains("security definer"))
    #expect(body.contains("coalesce(p_day, (now() at time zone 'Asia/Seoul')::date - 1)"))
...
func migrationContractMiniGameProbes() throws {
    ...
    #expect(sql.contains("if v_bal <> v_bal_a0 + 10 then"))
    #expect(sql.contains("if v_bal <> v_bal_a0 + 10 or jsonb_array_length(v_res->'awarded') <> 0 then"))
