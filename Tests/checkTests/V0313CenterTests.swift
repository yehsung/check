...
func 설정창_콘텐츠는_센터_행이_붙어도_창_안에_있다() throws {
    // 창 높이 계약(v0.3.22 부터 538pt — RealtimeLinkTests 의 그것과 같은 종류). 넘치면 **설정 항목 하나가 통째로 사라진다** —
    ...
    //   v0.3.22 실측(2026-09-15): '자동 근무 시작' 행이 붙어 533pt. 하한을 500 으로 올린다 — 센터 행(55pt)이
    //   빠지면 478 이라 여전히 걸리고, 새 행(68pt)이 빠져도 465 라 걸린다.
    #expect(height >= 500, "설정 콘텐츠가 \(height)pt 뿐이다 — 소속 센터 행이 사라졌는지 보라")
