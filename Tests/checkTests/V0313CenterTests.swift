...
func 설정창_콘텐츠는_센터_행이_붙어도_창_안에_있다() throws {
    // 창 높이 계약(v0.3.23 부터 648pt — RealtimeLinkTests 의 그것과 같은 종류). 넘치면 **설정 항목 하나가 통째로 사라진다** —
    ...
    //   v0.3.22 실측(2026-09-15): '자동 근무 시작' 행이 붙어 533pt. 하한을 500 으로 올렸다.
    //   v0.3.23 실측(2026-09-15): '근무 시작·종료 단축키' 스위치 + 키캡 기록 행이 붙어 624pt(이 렌더는 단축키 안내 한 줄이
    //   없는 상태다 — 한 줄이 보이는 가장 높은 상태 643pt 는 V0316 이 창과 비교한다). 하한을 600 으로 올린다 —
    //   센터 행(55pt)이 빠지면 569, 자동 근무 시작 행(68pt)이 빠지면 556, 단축키 묶음(91pt)이 빠지면 533,
    //   키캡 기록 행만(36pt) 빠져도 588 이라 전부 걸린다.
    #expect(height >= 600, "설정 콘텐츠가 \(height)pt 뿐이다 — 소속 센터 행이나 단축키 행이 사라졌는지 보라")
