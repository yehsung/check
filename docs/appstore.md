...
| 의료·웰니스 | 없음 | — |
| 도박(실제) | 아니오 | 결제 없음 |
| **모의 도박(Simulated Gambling)** | 아니오로 답하되 **확인 필요** | 오목에 루비 판돈(3/5/10)이 걸린다(사용자 결정 2026-09-16, `GamesText.swift:248` "건 루비는 돌려받아요"). 루비는 현금 구매·환금 불가(`privacy.md:103`) — 애플의 모의 도박 정의(카지노류 시뮬레이션)와 다르지만, 심사 노트에 "가상 포인트, 실물 가치 없음" 을 명시한다 |
| **경연(Contests)** | **확인 필요** | 미니게임 일별 1·2·3등에게 루비 20·10·5 지급(`GamesText.swift:80-83`). 상품이 앱 안 포인트뿐이라 애플 '경연' 정의(실물 상품·상금)와 다를 가능성이 크다 — 설문의 정의 문구를 보고 답한다 |
| 루트 상자 | 아니오 | 확률 뽑기 없음. 캐릭터는 정가 구매(`Me/MeText.swift:215`) |

예상 결과 등급: **4+**. 다만 앱이 성인 연수생 대상이고 별명·사진이 전원에게 보이므로, 설문의 "최소 연령을 더 높게 설정" 옵션으로 **13+** 를 고르는 것을 검토할 것(사용자 결정).
...
| 4 | **연령 설문 해석** — 오목 루비 판돈이 '모의 도박', 미니게임 루비 상품이 '경연' 으로 읽힐 수 있다 | 연령 등급 | §3 | 설문 정의를 읽고 답하되, 심사 노트에 "루비는 현금 구매·환금 불가한 앱 안 포인트" 명시(`privacy.md:103`) |
| 5 | **처리방침이 폰 현실과 어긋남** — `docs/privacy.md` 는 맥 중심이고 폰 절에 "가입을 받지 않습니다"(`:102`), "계정·기록 삭제는 제보로 요청"(`:118`) 이 적혀 있다. 작업 B·C 뒤엔 거짓이 되고, 지침 5.1.1(i) 은 보존·삭제 정책과 삭제 요청 방법을 요구한다 | 5.1.1(i) | `privacy.md:93-118` | 작업 B·C 병합 뒤 폰 절을 고친다: 폰 가입(별명·이메일·비밀번호·센터·팀 코드/팀 만들기), 앱 안 계정 삭제(무엇이 지워지고 제보는 익명화된다 — SPEC 서버 계약 `feedback_reports.user_id set null`), 빈 팀 자동 삭제. 그다음 §4 URL 로 게시하고 앱 설정에 링크 |
...
2. 위험 5·6·7 코드·문서 수정(별도 작은 작업) → `docs/privacy.md` 개정 → GitHub Pages 게시(§4) → 앱 설정에 처리방침 링크.
