...
* **배포 직후 하루는 한 번 헐거워진다.** 기존 사용자는 `ultra_quest_day` 가 null 이라 첫 sync 가
  baseline 을 0 으로 잡는다 — 오늘 이미 3시간 넘게 일한 사람은 그 자리에서 받을 수 있다(상한 3이
  잠그므로 최대 3개). 소멸 행을 지운 것과 합쳐 "새 규칙의 혜택을 오늘부터"라는 의도된 동작이다.
